...
ss2d.ParticleEmitter.MINUS_ONE_TO_ONE = function()
{
	return (Math.random() * 2) - 1;
}

ss2d.ParticleEmitter.prototype.generateParticle = function(particle, worldLocation)
{
	...
	particle.mTimeToLive = Math.max(0, this.mParticleLifeSpan + (this.mParticleLifespanVariance * m1to1()));
	...
	particle.mParticleSizeDelta = ((particleFinishSize - particleStartSize) / particle.mTimeToLive) * (1.0 / ss2d.ParticleEmitter.MAX_UPDATE_RATE);
	...
	deltaColor[0] = ((endColor[0] - startColor[0]) / particle.mTimeToLive) * (1.0 / ss2d.ParticleEmitter.MAX_UPDATE_RATE);
	deltaColor[1] = ((endColor[1] - startColor[1]) / particle.mTimeToLive) * (1.0 / ss2d.ParticleEmitter.MAX_UPDATE_RATE);
	deltaColor[2] = ((endColor[2] - startColor[2]) / particle.mTimeToLive) * (1.0 / ss2d.ParticleEmitter.MAX_UPDATE_RATE);
	deltaColor[3] = ((endColor[3] - startColor[3]) / particle.mTimeToLive) * (1.0 / ss2d.ParticleEmitter.MAX_UPDATE_RATE);
...
if(RENDER_CONTEXT == 'webgl')
{
	ss2d.ParticleEmitter.prototype.render = function(support)
	{
		...
		gl.drawElements(gl.TRIANGLES, this.mMaxParticles * support.mBuffers.mParticlesQVI.itemSize, gl.UNSIGNED_SHORT, 0);
	...
	ss2d.ParticleEmitter.prototype.updateParticles = function(deltaTime)
	{
		...
		for(var p_index = 0; p_index < this.mMaxParticles; ++p_index)
		{
			...
			var currentRadius = particle.mRadius - (particle.mRadiusDelta * (ss2d.CURRENT_VIEW.mTotalTime - particle.mTimeStamp));

			if(particle.mTimeToLive <= 0.0 || (this.mEmitterType == 1.0 && currentRadius < this.mMinRadius))
			{
				this.mParticleCount -= 1;
