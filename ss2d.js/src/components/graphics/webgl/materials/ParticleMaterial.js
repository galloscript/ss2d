...
ss2d.materials.Particle.prototype.updateParticle = function(buffer, data, offset)
{
...
ss2d.materials.Particle.VERTEX_SOURCE = ''+
	...
	'void main(void){'+
		...
		//'} else {'+
			...
			'pos += vec2(aPos_Dir.z, aPos_Dir.w) + direction;'+
			'pos += diff;'+
		...
		'gl_Position = vec4((uPMatrix * vec3((aVertexPosition * size * sizeToZero) + pos - vec2(size * 0.5, size * 0.5), 1.0)).xy, 0.0, 1.0);'+
	'}';
