...
ss2d.RenderSupport.prototype.pushTransform = "webgl" == RENDER_CONTEXT ? function(b) {
  ...
  this.mCurrentMatrix = this.mCurrentMatrix.concatMatrix(c)
} : function(b) {
  var c = b;
  b instanceof ss2d.DisplayObject && (c = b.getTransformationMatrix());
  this.mContext.save();
...
ss2d.Skeleton.allDegToRad = function(b) {
  ...
  for(d in b) {
    "rotation" == d || "angle" == d ? b[d] *= c : "y" == d ? b[d] *= -1 : "object" == typeof b[d] && ss2d.Skeleton.allDegToRad(b[d])
  }
};
ss2d.SkeletonLoader = {};
...
ss2d.ResourceManager.Loaders = {TEXTURE:ss2d.TextureLoader, SOUND:ss2d.AudioLoader, TEXTURE_ATLAS:ss2d.TextureAtlasLoader, BITMAP_FONT:ss2d.BitmapFontLoader, REELSET:ss2d.ReelSetLoader, SKELETON:ss2d.SkeletonLoader, SKELETAL_ANIMATION:ss2d.SkeletalAnimationLoader};
...
ss2d.BitmapTextSprite = function(b, c, d, e, f) {
  ...
  if(COMPILING_CLIENT || COMPILING_OFFLINE) {
    this.mGlyphSprite = new ss2d.Sprite, this.mClip = [], this.mDisplacement = new ss2d.Point
  }
};
...
if(COMPILING_CLIENT || COMPILING_OFFLINE) {
  ss2d.TextSprite.prototype.render = function(b) {
    b.pushTransform(this);
    var c = b.mContext;
...
ss2d.ClientView.prototype.nextFrame = function() {
  ...
  null != this.mMainScene && this.mMainScene.render(this.mRenderSupport);
...
ss2d.Bone = function(b, c, d, e, f, g, h) {
  ss2d.DisplayObjectContainer.call(this, d, e, f, g);
  this.mLength = h;
  this.mName = c;
  ...
  this.mSetupPose = {x:d, y:e, s:f, r:g}
};
goog.inherits(ss2d.Bone, ss2d.DisplayObjectContainer);
ss2d.Bone.prototype.interpolateBoneStates = function(b, c, d, e) {
  this.mLocation.mX = this.mSetupPose.x + b.x + (c.x - b.x) * d;
  this.mLocation.mY = this.mSetupPose.y + b.y + (c.y - b.y) * d;
  this.mRotation = this.mSetupPose.r + b.r + (c.r - b.r) * e
};
ss2d.Bone.prototype.tick = function(b) {
  ...
  if(b.mCurrentAnimation) {
    ...
    if(c && c.bones[this.mName]) {
      b = 0 > b.mTimeDilation ? !0 : !1;
      var d = this.mParentSkeletalSprite.mCurrentAnimationTime, e = c.bones[this.mName].rotate, c = c.bones[this.mName].translate, f = {x:0, y:0, r:0}, g = {x:0, y:0, r:0}, h = null, j = null, k = null, l = null, m = 1, n = 1;
      if(e) {
        for(var q = 0;q < e.length && null == h;q++) {
          e[q].time > d && (h = e[q], j = 0 == q ? e[e.length - 1] : e[q - 1])
        }
        j && h && (f.r = j.angle, g.r = h.angle, n = (d - j.time) / (h.time - j.time))
      }
      if(c) {
        for(e = 0;e < c.length && null == k;e++) {
          c[e].time > d && (k = c[e], l = 0 == e ? c[c.length - 1] : c[e - 1])
        }
        l && k && (f.x = l.x, g.x = k.x, f.y = l.y, g.y = k.y, m = (d - l.time) / (k.time - l.time))
      }
      b ? this.interpolateBoneStates(g, f, 1 - m, 1 - n) : this.interpolateBoneStates(f, g, m, n)
    }
  }
};
...
ss2d.Slot.prototype.render = function(b) {
...
ss2d.SkeletalSprite = function(b, c, d, e, f) {
...
ss2d.SkeletalSprite.prototype.setup = function(b) {
  ...
  for(var c in b) {
    ...
    var f = new ss2d.Bone(this, d.name, d.x || 0, d.y || 0, d.scale || 1, d.rotation || 0, d.length || 0);
  ...
  for(var g in c) {
    d = c[g], d.attachment && (e = b[d.name][d.attachment], f = new ss2d.Slot(this.mBoneMap[d.bone], d.name, e.x || 0, e.y || 0, e.width || 1, e.height || 1, d.attachment, this.mBodyAtlas), f.mPivotX = 0.5 * f.mWidth, f.mPivotY = 0.5 * f.mHeight, f.mRotation = e.rotation || 0, this.mSlotMap[d.name] = f, this.mSlots.addObject(f))
  }
};
...
ss2d.SkeletalSprite.prototype.updateAnimation = function(b) {
  this.mCurrentAnimation && 0.01 < Math.abs(this.mTimeDilation) && (this.mCurrentAnimationTime += b * this.mTimeDilation, 0 < this.mTimeDilation && this.mCurrentAnimationTime >= this.mCurrentAnimation.mDuration ? this.mCurrentAnimationTime -= this.mCurrentAnimation.mDuration : 0 > this.mTimeDilation && 0 >= this.mCurrentAnimationTime && (this.mCurrentAnimationTime += this.mCurrentAnimation.mDuration), this.tickChildren(b));
