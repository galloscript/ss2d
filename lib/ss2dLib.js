...
ss2d.materials.GPUParticle = function(b) {
  ...
  this.mShaderProgram = new ss2d.ShaderProgram([b, c], "uPMatrix uISMatrix uTotalTime uEmitterType uGravity uDeltaTime".split(" "), "aVertexPosition aPos_Dir aStartPos aColor aDeltaColor aRo_RoD_RaA_TaA aRa_RaD_A_DPS aPS_PSD_T_TS".split(" "));
...
ss2d.materials.GPUParticle.prototype.apply = function(b) {
  ...
  c.uniformMatrix3fv(this.mShaderProgram.mUniforms.uISMatrix, !1, ss2d.CURRENT_VIEW.mMainScene.getTransformationMatrix().getMatF32Array(this.mAuxF32Matrix));
...
ss2d.Defines = {};
var RENDER_CONTEXT = "2d", COMPILING_CLIENT = !1, COMPILING_SERVER = !1, COMPILING_OFFLINE = !0, COMPILING_ADVANCE = !1;
...
ss2d.HTML5Audio = function(b, c) {
  ...
  this.mAudioElement.mCallbackFunction = c || function() {
  };
  c && this.mAudioElement.addEventListener("canplay", function() {
...
ss2d.Input = function(b) {
  ...
  if(navigator.userAgent.match(/Android/i) || navigator.userAgent.match(/iPhone/i) || navigator.userAgent.match(/iPod/i) || navigator.userAgent.match(/iPad/i)) {
    this.mMobileDevice = !0, b.style.webkitTapHighlightColor = "rgba(0,0,0,0)"
  }
...
ss2d.Input.prototype.getTouch = function(b, c) {
  if(c && c.targetTouches) {
    return c.targetTouches[b] || !1
  }
  if(window.event && window.event.targetTouches) {
    return window.event.targetTouches[b] || !1
  }
};
ss2d.Input.prototype.onMouseDown = function(b) {
  var c = this.mMobileDevice ? this.getTouch(0, b) : b;
  if(!c) {
    return!1
  }
...
ss2d.Input.prototype.onMouseMove = function(b) {
  if(b = this.mMobileDevice ? this.getTouch(0, b) : b) {
...
ss2d.Skeleton.allDegToRad = function(b) {
  ...
  for(d in b) {
    "rotation" == d || "angle" == d ? (180 < b[d] ? b[d] -= 360 : -180 > b[d] && (b[d] += 360), b[d] *= c) : "y" == d ? b[d] *= -1 : "object" == typeof b[d] && "scale" != d && ss2d.Skeleton.allDegToRad(b[d])
  }
};
ss2d.SkeletonLoader = {};
...
ss2d.BitmapFont = function(b, c, d) {
  this.mName = b;
  this.mCallbackFunction = c;
  this.mCallbackTarget = d;
...
ss2d.ResourceManager.loadResources = function(b, c, d) {
  if(b instanceof Array) {
    ...
    ss2d.ResourceManager.CURRENT_ELEMENT_TO_LOAD = 0;
    0 == b.length && (ss2d.ResourceManager.ELEMENTS_LOADED_CALLBACK.call(), ss2d.ResourceManager.ELEMENTS_LOADED_CALLBACK = function() {
    }, ss2d.ResourceManager.ONE_ELEMENT_LOADED_CALLBACK = function() {
    });
    c = 0;
    for(resKey in b) {
...
ss2d.Sprite = function(b, c, d, e, f, g) {
  ...
  if(COMPILING_CLIENT || COMPILING_OFFLINE) {
    this.mTextureAtlas = g || null, this.mTexture = f || null, this.mReady = !1, "string" == typeof g ? g = ss2d.ResourceManager.loadTextureAtlas(g, function(b) {
      this.mTextureAtlas = b;
      this.mReady = !0
    }, this) : "string" == typeof f && !g && (f = ss2d.ResourceManager.loadTexture(f, function(b) {
      this.mTexture = b;
      this.mReady = !0
    }, this))
  }
...
if(COMPILING_CLIENT || COMPILING_OFFLINE) {
  ss2d.Sprite.prototype.render = "webgl" == RENDER_CONTEXT ? function(b) {
    if(this.mReady && !(this.mTextureAtlas && -1 == this.mTextureAtlas.mTextureId) && (this.mTextureAtlas || -1 != this.mTexture.mTextureId)) {
      var c = this.mTexture;
      this.mTextureAtlas && (c = this.mTextureAtlas.mTexture) && this.mTextureAtlas.mAtlasDescriptor && this.mTextureAtlas.getClipFor(this.mTexture, this.mClip);
      if(c && c.mTextureElement) {
...
ss2d.BitmapTextSprite = function(b, c, d, e, f, g) {
  ss2d.Quad.call(this, b, c, 0, 0, g);
  this.mTextString = d || "";
  this.mFontSize = f || 16;
  this.mDisplayChars = -1;
  if(COMPILING_CLIENT || COMPILING_OFFLINE) {
    this.mGlyphSprite = new ss2d.Sprite, this.mClip = [], this.mDisplacement = new ss2d.Point, this.mBitmapFont = ss2d.ResourceManager.loadBitmapFont(e, function() {
...
ss2d.Tween.prototype.scaleTo = function(b) {
  this.animateProperty(this.mTarget.getScaleX, this.mTarget.setScaleX, b);
  this.animateProperty(this.mTarget.getScaleY, this.mTarget.setScaleY, b)
};
...
ss2d.GPUParticleEmitter.prototype.setup = function(b) {
  ...
  this.mParticles = [];
  b = [];
  for(var c = [], d = this.localToWorld(this.mLocation), d = ss2d.CURRENT_VIEW.mMainScene.getTransformationMatrix().invert().transformPoint(d), e = 0;e < this.mMaxParticles;++e) {
...
"webgl" == RENDER_CONTEXT && (ss2d.GPUParticleEmitter.prototype.render = function(b) {
  ...
  if(this.mReady) {
    ...
    if(this.mActive && this.mEmissionRate) {
      ...
      this.mEmitCounter += b;
      for(var e = [], f = this.localToWorld(this.mLocation), f = ss2d.CURRENT_VIEW.mMainScene.getTransformationMatrix().invert().transformPoint(f);this.mParticleCount < this.mMaxParticles && this.mEmitCounter > d;) {
...
ss2d.View.prototype.nextFrame = function() {
  var b = (new Date).getTime(), c = b - this.mLastFrameTimestamp;
  this.resizeCanvas(this.mCanvas.width, this.mCanvas.height);
  for(var d in this.mPreRenderFunctions) {
    this.mPreRenderFunctions[d].call(null, this.mRenderSupport)
  }
  this.render();
  for(d in this.mPostRenderFunctions) {
  ...
  for(var e = Math.floor(Math.max(1, c / (1E3 / ss2d.PhysicalWorld.UPDATE_RATE))), f = 0;f < e;++f) {
    this.mPhysicalWorld.tick(c / e / 1E3)
  }
  c /= 1E3;
  this.mRealFPS = 0.01 * Math.floor(100 / c);
  this.mTotalTime += c;
  this.mInput.tick(c);
  for(d in this.mPreTickFunctions) {
    this.mPreTickFunctions[d].call(null, c)
  }
...
ss2d.SkeletalSprite.prototype.playAnimation = function(b, c, d) {
  this.mCurrentAnimationName = b;
  this.mCurrentAnimationTime = c || 0;
  this.mPlayOnce = d || !1;
  this.mCurrentAnimation = this.mAnimationsMap[b] || null
};
ss2d.SkeletalSprite.prototype.addTrigger = function(b, c, d) {
  this.mTriggers[b] || (this.mTriggers[b] = []);
  this.mTriggers[b].push({time:c, callback:d})
};
...
ss2d.SkeletalSprite.prototype.updateAnimation = function(b) {
  var c = this.mCurrentAnimationTime;
  if(this.mCurrentAnimation && 0.01 < Math.abs(this.mTimeDilation)) {
    this.mCurrentAnimationTime += b * this.mTimeDilation;
    var d = !1;
    if(0 < this.mTimeDilation) {
      this.mCurrentAnimationTime >= this.mCurrentAnimation.mDuration && (this.mCurrentAnimationTime -= this.mCurrentAnimation.mDuration, d = !0);
      for(var e in this.mTriggers[this.mCurrentAnimationName]) {
        var f = this.mTriggers[this.mCurrentAnimationName][e];
        f.time > c && (f.time < this.mCurrentAnimationTime || d) && f.callback.call(this)
      }
    }
    if(0 > this.mTimeDilation) {
      for(e in 0 >= this.mCurrentAnimationTime && (this.mCurrentAnimationTime += this.mCurrentAnimation.mDuration, d = !0), this.mTriggers[this.mCurrentAnimationName]) {
        f = this.mTriggers[this.mCurrentAnimationName][e], f.time < c && (f.time > this.mCurrentAnimationTime || d) && f.callback.call(this)
      }
    }
    d && this.mPlayOnce && this.stopAnimation()
  }
  this.tickChildren(b);
...
ss2d.ParticleEmitter.prototype.setup = function(b) {
  ...
  b = ss2d.CURRENT_VIEW.mMainScene.getTransformationMatrix().invert().transformPoint(b);
