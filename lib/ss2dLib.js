...
ss2d.RigidBody.prototype.reset = function() {
  ...
  switch(this.mBodyType) {
    case ss2d.RigidBody.Types.BOX:
      ...
      this.mOwner.mPivotX = 0.5 * c.mWidth / this.mOwner.mScaleX;
      this.mOwner.mPivotY = 0.5 * c.mHeight / this.mOwner.mScaleY;
...
ss2d.DisplayObjectContainer.prototype.getBounds = function() {
  ...
  return new ss2d.Rectangle(c, d, e - c, f - d)
};
...
if(COMPILING_CLIENT || COMPILING_OFFLINE) {
  ss2d.BitmapTextSprite.prototype.render = function(b) {
    ...
    try {
      ...
      for(var e = -1 < this.mDisplayChars ? Math.min(this.mDisplayChars, this.mTextString.length) : this.mTextString.length, f = 0;f < e;++f) {
        var g = this.mTextString.charCodeAt(f);
        32 == g ? c.translate(0.5 * this.mBitmapFont.mMidWidth, 0) : (this.displacement(f, g, d, this.mClip[5], this.mDisplacement), this.mBitmapFont.getGlyphClip(g, this.mClip), this.mGlyphSprite.mLocation.mX = this.mDisplacement.mX, this.mGlyphSprite.mLocation.mY = this.mDisplacement.mY, this.mGlyphSprite.mClip = this.mClip, this.mGlyphSprite.mWidth = this.mClip[2], this.mGlyphSprite.mHeight = this.mClip[3], this.mGlyphSprite.render(b), d += this.mClip[4])
      }
    }catch(h) {
    }
    b.popTransform()
...
ss2d.PhysicalWorld = function() {
  ...
  b.minVertex.Set(-1024, -1024);
  b.maxVertex.Set(1024, 1024);
...
ss2d.View.prototype.nextFrame = function() {
  ...
  this.mInput.tick(c / 1E3);
  for(var d = Math.floor(Math.max(1, c / (1E3 / this.mFrameRate))), e = 0;e < d;++e) {
...
ss2d.Pickable.prototype.tick = function() {
  var b = ss2d.CURRENT_VIEW.mInput;
  this.mJustPicked = !1;
  if(b.mMouseDown && !b.mPreviousMouseDown && this.mOwner.hitTestPoint(b.mMousePoint)) {
    ss2d.Pickable.PICKED_COMPONENT && (this.mMouseJoint && ss2d.PhysicalWorld.getWorld().mWorld.DestroyJoint(this.mMouseJoint), this.mMouseJoint = null, ss2d.Pickable.PICKED_COMPONENT.mPicked = !1);
    this.mPicked = this.mJustPicked = !0;
    ss2d.Pickable.PICKED_COMPONENT = this;
    var c = this.mOwner.worldToLocal(b.mMousePoint);
    this.mOffset.set(this.mOwner.mLocation.mX - c.mX, this.mOwner.mLocation.mY - c.mY);
    this.mRigidBody && (this.mMouseJoint = ss2d.PhysicalWorld.getWorld().createMouseJoint(this.mRigidBody), this.mRigidBody.mBody.WakeUp())
  }
  (this.mPicked = ss2d.Pickable.PICKED_COMPONENT == this && b.mMouseDown) ? this.mMoveObject && (c = this.mOwner.worldToLocal(b.mMousePoint), this.mRigidBody ? this.mMouseJoint && this.mMouseJoint.m_target.Set(c.mX, c.mY) : (this.mOwner.mLocation.mX = c.mX + this.mOffset.mX, this.mOwner.mLocation.mY = c.mY + this.mOffset.mY)) : ss2d.Pickable.PICKED_COMPONENT == this && (ss2d.Pickable.PICKED_COMPONENT = null, ss2d.PhysicalWorld.getWorld().mWorld.DestroyJoint(this.mMouseJoint), this.mMouseJoint = null);
